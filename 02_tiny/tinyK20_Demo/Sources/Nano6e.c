...
uint8_t nanoGetTagEPCBytes(void)
{
...
//Often this is zero
uint8_t nanoGetTagDataBytes(void)
{
...
uint16_t nanoGetTagTimestamp(void)
{
...
uint8_t nanoParseResponse(void)
{
...
void nanoSendMessage(uint8_t opcode, uint8_t *data, uint8_t size, uint16_t timeOut, boolean waitForResponse)
{
...
void nanoSendCommand(uint16_t timeOut, boolean waitForResponse)
{
  ...
  {
	//übernimmt das nicht die HW?//_nanoSerial->flush(); //Wait for serial sending to complete
	return;
  }
